...
class BloodLipids(BaseModel):
    ...
    total_cholesterol: float = Field(
        ..., ge=0, le=50, description="总胆固醇 mmol/L"
    )
    triglycerides: float = Field(
        ..., ge=0, le=50, description="甘油三酯 mmol/L"
    )
    hdl: float = Field(
        ..., ge=0, le=50, description="HDL mmol/L"
    )
    ldl: float = Field(
        ..., ge=0, le=50, description="LDL mmol/L"
    )
...
class BloodGlucose(BaseModel):
    ...
    fasting_glucose: float = Field(
        ..., ge=0, le=50, description="空腹血糖 mmol/L"
    )
    hba1c: float = Field(
        ..., ge=0, le=20, description="糖化血红蛋白 %"
    )
...
class Lifestyle(BaseModel):
    ...
    sleep_duration: float = Field(
        ..., ge=0, le=24, description="睡眠时长(小时)"
    )
    exercise_frequency: ExerciseFrequency = Field(
        ..., description="运动频率"
    )
    diet_habit: DietHabit = Field(
        ..., description="饮食习惯"
    )
    stress_level: int = Field(
        ..., ge=1, le=10, description="压力水平 1-10"
    )
...
class UserProfile(BaseModel):
    ...
    height: float = Field(
        ..., ge=30, le=300, description="身高(厘米)"
    )
    weight: float = Field(
        ..., ge=0.5, le=500, description="体重(千克)"
    )
