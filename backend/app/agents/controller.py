...
请根据用户的查询内容和健康画像，生成针对性的子任务描述。"""
    ...
    def _handle_prohibited_content(self, state: HealthState, safety_result: dict) -> HealthState:
        """处理检测到禁止关键词的情况
    ...
    def _handle_out_of_scope(self, state: HealthState, safety_result: dict) -> HealthState:
        """处理超出服务范围的请求
    ...
    def _handle_high_risk_content(self, state: HealthState, safety_result: dict) -> HealthState:
        """处理检测到高风险关键词的情况
    ...
    def _create_task_breakdown(self, user_query: str, user_profile) -> list[SubTask]:
        """创建任务拆解
    ...
    def _extract_profile_context(self, user_profile) -> dict:
        ...
        if hasattr(user_profile, "gender"):
            context["gender"] = (
                str(user_profile.gender.value)
                if hasattr(user_profile.gender, "value")
                else str(user_profile.gender)
            )
        ...
        if hasattr(user_profile, "medications") and user_profile.medications:
            context["medications"] = [
                m.drug_name if hasattr(m, "drug_name") else str(m) for m in user_profile.medications
            ]
    ...
    def _generate_nutrition_task_description(self, user_query: str, profile_context: dict) -> str:
        """生成营养子任务描述
    ...
    def _generate_controller_response(
        ...
        if safety_result.get("ui_interrupt_flag"):
            response_parts.append("注意：检测到高风险内容，需要用户确认后继续。")
    ...
    def _update_node_status(self, state: HealthState, status: NodeExecutionStatus) -> HealthState:
        """更新节点执行状态
