...
class SSEManager:
            ...
            while self._active_sessions.get(session_id, False):
                try:
                    ...
                    event = await asyncio.wait_for(
                        self._event_queues[session_id].get(), timeout=1.0
                    )
    ...
    async def emit_event(self, session_id: str, event: SSEEvent) -> None:
        """发送 SSE 事件
    ...
    async def emit_status(
        self, session_id: str, agent_name: str, progress: str, percentage: int
    ) -> None:
        """发送状态事件的便捷方法
        ...
        event = SSEEvent(
            ...
            data=StatusEventData(agent_name=agent_name, progress=progress, percentage=percentage),
        )
        await self.emit_event(session_id, event)

    async def emit_intermediate(self, session_id: str, data: Any) -> None:
        """发送中间结果事件的便捷方法
        ...
        Args:
            ...
            data: 中间结果数据
        """
        event = SSEEvent(event_type=SSEEventType.INTERMEDIATE, data=data)
        await self.emit_event(session_id, event)

    async def emit_report(self, session_id: str, report: Any) -> None:
        """发送最终报告事件的便捷方法
        ...
        """
        event = SSEEvent(event_type=SSEEventType.REPORT, data=report)
        await self.emit_event(session_id, event)

    async def emit_pause(
        self, session_id: str, high_risk_items: list, timeout_seconds: int = 600
    ) -> None:
        """发送暂停事件的便捷方法
    ...
    async def emit_error(self, session_id: str, error_type: str, error_message: str) -> None:
        """发送错误事件的便捷方法
        ...
        event = SSEEvent(
            ...
            data=ErrorEventData(error_type=error_type, error_message=error_message),
        )
        await self.emit_event(session_id, event)
        ...
        while self._active_sessions.get(session_id, False):
            try:
                ...
                heartbeat_event = SSEEvent(
                    event_type=SSEEventType.HEARTBEAT, data={"message": "keepalive"}
                )
