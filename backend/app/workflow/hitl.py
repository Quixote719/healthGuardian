...
class PausedSession:
    ...
    def __init__(
        ...
        state: HealthState,
        reason: str,
        high_risk_items: Optional[List[Any]] = None,
    ):
...
class HITLManager:
    ...
    async def check_timeout(
        ...
        # 使用指定的超时时间或默认的最大暂停时间
        effective_timeout = timeout_minutes if timeout_minutes is not None else self.max_pause_minutes
    ...
    def get_session(self, session_id: str) -> Optional[PausedSession]:
        """获取暂停会话信息
    ...
    async def terminate_session(self, session_id: str, reason: str = "manual termination") -> Optional[HealthState]:
        """终止会话
    ...
    async def cleanup_timeout_sessions(self) -> List[str]:
        ...
        for session_id in session_ids:
            if await self.check_timeout(session_id):
                ...
                await self.terminate_session(
                    session_id,
                    reason=f"timeout after {self.max_pause_minutes} minutes"
                )
        ...
        if terminated_sessions:
            logger.info(f"Cleaned up {len(terminated_sessions)} timeout sessions: {terminated_sessions}")
