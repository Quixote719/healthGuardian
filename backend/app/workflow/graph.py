...
async def with_timeout_and_error_handling(
    ...
    try:
        ...
        logger.warning(
            f"Agent {node_name} timed out after {timeout_seconds} seconds"
        )
...
def create_parallel_health_workflow() -> StateGraph:
    ...
    def route_after_controller(state: HealthState) -> Literal["interrupt", "nutrition", "rehabilitation", "neuropsychology"]:
        """路由函数，返回单个目标"""
        if state.get("ui_interrupt_flag", False):
            return "interrupt"
        task_breakdown = state.get("task_breakdown", [])
...
async def resume_health_workflow(
    ...
    update_state: Optional[HealthState] = None,
) -> HealthState:
    """恢复暂停的健康咨询工作流
